...
import { UserPlus, Users, Search, Settings, ArrowLeft } from 'lucide-react';
...
const ClientAssignment = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch agents from profiles with agent role
      ...
      // Fetch related client and agent data separately
      const transformedAssignments: ClientAssignment[] = [];
      for (const assignment of assignmentsData || []) {
        const { data: clientData } = await supabase
          .from('clients')
          .select('id, first_name, last_name, email, phone, company, user_id')
      ...
      console.error('Failed to load client assignment data:', error);
      toast({
        title: "Error",
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-muted-foreground">
              You need manager or supervisor privileges to access client assignment features.
            </p>
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Client Assignment</h1>
                ...
                <div key={assignment.id} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="space-y-1">
                    ...
                    {assignment.client.company && (
                      <div className="text-xs text-muted-foreground">{assignment.client.company}</div>
                    )}
                  </div>
