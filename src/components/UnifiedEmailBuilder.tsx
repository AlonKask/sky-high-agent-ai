...
export default function UnifiedEmailBuilder({ 
  ...
  const generateEmailHTML = async (): Promise<string> => {
    ...
    const buildOptionCard = (quote: Quote) => {
      ...
      // Format departure date from first segment or quote
      const formatSegmentDate = (segment: any) => {
      ...
      const departureDate = formatSegmentDate(first);

      // Build passenger price breakdown
      const passengerBreakdown: string[] = [];
      if (paxAdults > 0 && adultPrice) passengerBreakdown.push(`${paxAdults}×ADT $${fmtNum(adultPrice)}`);
      if (paxChildren > 0 && childPrice) passengerBreakdown.push(`${paxChildren}×CHD $${fmtNum(childPrice)}`);
      const priceBreakdown = passengerBreakdown.join(' · ');

      // Build segment rows
      const segmentRows = (segs || []).map((segment: any) => {
        const flightNumber = segment.flightNumber || 'TBD';
        const cabin = segment.cabin || segment.cabinClass || 'Business';
        const rbd = segment.bookingClass || 'J';
        const depTime = segment.departureTime || 'TBD';
        const arrTime = segment.arrivalTime || 'TBD';
        const depPort = segment.departureAirport || segment.origin || 'TBD';
        const arrPort = segment.arrivalAirport || segment.destination || 'TBD';
        const aircraft = segment.aircraft || 'TBD';
        const duration = segment.duration ? `${Math.floor(segment.duration / 60)}h ${segment.duration % 60}m` : 'TBD';
        const segmentDate = formatSegmentDate(segment);
        ...
        return `
          <tr>
            ...
            <td width="36%" style="padding:10px 12px;vertical-align:top;border-top:1px dashed #D0D6E0;">
              <div>
                <span style="font-family:ui-monospace,Menlo,Consolas,'Courier New',monospace;">${depTime}</span> ${depPort} →
                <span style="font-family:ui-monospace,Menlo,Consolas,'Courier New',monospace;">${arrTime}</span> ${arrPort}
              </div>
              <div style="font-size:12px;color:#6B7280;">${segmentDate}</div>
            </td>
      ...
      const totalDuration = quote.parsedItinerary?.totalDuration || formatDuration(segs) || 'TBD';
      const stopText = stops === 0 ? 'Direct' : stops === 1 ? '1 stop' : `${stops} stops`;
      const layoverInfo = stops > 0 && segs.length > 1 ? `(${segs[0]?.arrivalAirport || segs[0]?.destination || 'TBD'})` : '';
              ...
              <table role="presentation" width="100%">
                <tr>
                  <td style="font-weight:700;font-size:18px;">${depCode} → ${arrCode}
                    <div style="font-size:12px;color:#6B7280;margin-top:2px;">${departureDate} • Local time</div>
                  </td>
                  <td align="right">
                    <div style="font-family:ui-monospace,Menlo,Consolas,'Courier New',monospace;font-size:20px;font-weight:700;">$${fmtNum(totalPrice)}</div>
                    ${priceBreakdown ? `<div style="font-size:12px;color:#6B7280;font-family:ui-monospace,Menlo,Consolas,'Courier New',monospace;">${priceBreakdown}</div>` : ''}
                  </td>
                </tr>
              </table>
                ...
                <tr>
                  <td colspan="3" style="padding:10px 12px;">
                    ...
                    <div style="font-size:12px;color:#6B7280;margin-top:8px;">
                      ${stopText} ${layoverInfo} • Total ${totalDuration} • Baggage 2×23kg
                    </div>
                  </td>
