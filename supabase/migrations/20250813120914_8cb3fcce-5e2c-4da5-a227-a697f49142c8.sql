-- ===============================================
-- COMPREHENSIVE SECURITY REMEDIATION MIGRATION (CORRECTED)
...
BEGIN
  ...
  IF user_role IN ('manager', 'supervisor') THEN
    ...
    IF is_team_member THEN
      PERFORM public.log_security_event(
        ...
        'medium',
        jsonb_build_object(
          'event_subtype', 'financial_data_access',
          'accessing_user_id', accessing_user_id,
          'target_user_id', target_user_id,
          'role', user_role,
  ...
  IF user_role = 'admin' THEN
    PERFORM public.log_security_event(
      ...
      'high',
      jsonb_build_object(
        'event_subtype', 'admin_financial_access',
        'admin_id', accessing_user_id,
        'target_user_id', target_user_id,
        'requires_justification', true
      )
  ...
  IF accessing_user_id != target_user_id THEN
    ...
    IF user_role = 'admin' THEN
      PERFORM public.log_security_event(
        ...
        'critical',
        jsonb_build_object(
          'event_subtype', 'admin_session_access',
          'admin_id', accessing_user_id,
          'target_user_id', target_user_id,
          'requires_justification', true
        )
    ...
    PERFORM public.log_security_event(
      ...
      'critical',
      jsonb_build_object(
        'event_subtype', 'unauthorized_session_access',
        'accessing_user_id', accessing_user_id,
        'target_user_id', target_user_id
      )
    );
...
BEGIN
  ...
  AND event_type IN ('failed_authorization', 'suspicious_activity')
  AND details->>'event_subtype' IN ('unauthorized_client_access_denied', 'unauthorized_session_access', 'gmail_credentials_unauthorized_access')
  ...
  IF recent_failures >= 3 THEN
    ...
    VALUES (
      ...
      'critical',
      jsonb_build_object(
        'event_subtype', 'suspicious_activity_detected',
...
CREATE TRIGGER detect_suspicious_activity_trigger
  ...
  FOR EACH ROW
  WHEN (NEW.event_type IN ('failed_authorization', 'suspicious_activity'))
...
AS $$
BEGIN
  -- Mark expired sessions as inactive if table exists
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_sessions') THEN
