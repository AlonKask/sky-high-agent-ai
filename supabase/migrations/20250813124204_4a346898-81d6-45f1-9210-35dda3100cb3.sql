-- SECURITY HARDENING: Implement missing RLS policies for exposed tables

-- 1. CREATE MISSING TABLES AND APPLY RLS POLICIES
...
-- 2. DROP ALL EXISTING POLICIES AND RECREATE WITH ENHANCED SECURITY
...
-- 3. FIX SECURITY EVENTS TABLE CHECK CONSTRAINT

-- Get current check constraint details
DO $$
DECLARE
    constraint_definition text;
BEGIN
    -- Check what event types are currently allowed
    SELECT pg_get_constraintdef(oid) INTO constraint_definition
    FROM pg_constraint 
    WHERE conname = 'security_events_event_type_check';
    
    -- Drop and recreate with comprehensive event types
