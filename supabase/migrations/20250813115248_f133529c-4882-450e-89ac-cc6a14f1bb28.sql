...
-- Phase 4: Add the audit trigger to clients table (only for DML operations)
...
-- Phase 7: Create data access monitoring view for admins
CREATE OR REPLACE VIEW public.client_access_audit AS
SELECT 
  se.id,
  se.user_id as accessing_user_id,
  p.first_name || ' ' || p.last_name as accessing_user_name,
