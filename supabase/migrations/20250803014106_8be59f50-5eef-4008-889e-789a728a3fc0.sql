-- Enhanced airline data with comprehensive IATA/ICAO codes (only insert if not exists)
INSERT INTO public.airline_codes (iata_code, icao_code, name, country, alliance, logo_url) 
SELECT * FROM (VALUES
-- Major US Airlines (excluding AA which already exists)
...
) AS new_airlines(iata_code, icao_code, name, country, alliance, logo_url)
WHERE NOT EXISTS (
    SELECT 1 FROM public.airline_codes 
    WHERE airline_codes.iata_code = new_airlines.iata_code
);

-- Enhanced airport data with comprehensive IATA/ICAO codes and coordinates
INSERT INTO public.airport_codes (iata_code, icao_code, name, city, country, latitude, longitude, timezone) 
SELECT * FROM (VALUES
-- Major US Airports
...
) AS new_airports(iata_code, icao_code, name, city, country, latitude, longitude, timezone)
WHERE NOT EXISTS (
    SELECT 1 FROM public.airport_codes 
    WHERE airport_codes.iata_code = new_airports.iata_code
);
