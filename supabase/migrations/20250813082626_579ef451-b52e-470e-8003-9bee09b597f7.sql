...
-- Phase 1: Restrict Public Data Access

-- 1. Update airline_codes RLS policies to require authentication
...
-- 2. Update airport_codes RLS policies to require authentication  
...
-- 3. Remove plain text OAuth token columns from gmail_credentials
ALTER TABLE public.gmail_credentials 
DROP COLUMN IF EXISTS access_token,
DROP COLUMN IF EXISTS refresh_token;

-- 4. Add trigger to prevent accidental storage of plain text tokens
...
-- 5. Enhance option_reviews with secure token generation
...
-- 6. Add token expiration to option_reviews
...
-- 7. Update option_reviews token generation to use secure tokens
...
-- 8. Enhanced get_option_reviews_by_token function with security checks
...
-- 9. Add rate limiting for public access functions
