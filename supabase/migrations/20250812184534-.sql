...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Admins and managers can view security events' AND tablename = 'security_events'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'System can insert security events' AND tablename = 'security_events'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Users can log sensitive data access' AND tablename = 'sensitive_data_access'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Admins and managers can view sensitive data access' AND tablename = 'sensitive_data_access'
  ) THEN
...
BEGIN
  ...
  IF NEW.encrypted_payment_info IS NOT NULL THEN
    IF jsonb_typeof(NEW.encrypted_payment_info) = 'string' THEN
      IF NOT public.is_base64(NEW.encrypted_payment_info::text::jsonb #>> '{}') THEN
        -- Convert jsonb string to text then validate
        IF NOT public.is_base64(trim(both '"' from NEW.encrypted_payment_info::text)) THEN
