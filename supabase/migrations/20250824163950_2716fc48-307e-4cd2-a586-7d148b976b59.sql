-- First, let's create a proper get_analytics_data function that returns all needed fields
...
BEGIN
  ...
  -- Calculate metrics
  avg_ticket := CASE WHEN quote_count > 0 THEN revenue_sum / quote_count ELSE 0 END;
  ...
  FROM (
    SELECT 
      ...
      COALESCE(SUM(q.total_price), 0) AS month_revenue,
  ...
  FROM (
    SELECT 
      ...
      SUM(q.total_price) as route_revenue
    FROM public.quotes q
    WHERE q.created_at::date BETWEEN start_date_parsed AND end_date_parsed
      AND (p_agent_id IS NULL OR q.user_id = p_agent_id)
      AND q.route IS NOT NULL
    GROUP BY q.route
    ORDER BY route_count DESC
    LIMIT 10
  ) route_stats;

  -- Generate agent performance (if not filtering by specific agent)
  IF p_agent_id IS NULL THEN
    ...
    FROM (
      SELECT 
        ...
        COALESCE(SUM(q.total_price), 0) as agent_revenue
      FROM public.quotes q
      LEFT JOIN public.bookings b ON q.user_id = b.user_id 
