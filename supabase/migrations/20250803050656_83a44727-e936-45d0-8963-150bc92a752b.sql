-- Phase 2: Add RBD assignments for all airlines
-- Create RBD assignments systematically for all service classes

-- Add Economy Y booking class for ALL airlines (single class for LCCs, part of multi-class for FSCs)
...
WHERE NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'Y'
);

-- Add Business class J for airlines that offer business class (exclude LCCs and all-economy airlines)
INSERT INTO public.airline_rbd_assignments (airline_id, booking_class_code, service_class, class_description, booking_priority, is_active)
SELECT 
    ac.id,
    'J',
...
WHERE ac.iata_code NOT IN (
    -- Exclude LCCs and single-class airlines
    'AQ', 'G9', 'BX', 'IX', 'I5', 'PA', 'G4', 'F9', 'AK', 'I5', 'U2', '5J', '6E', 'JQ', 'FR', 'NK', 'SG', '9C', 'VJ', 'W6', 'Y4', 'WN', 'PG'
) 
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'J'
);

-- Add Business class C for full-service carriers
INSERT INTO public.airline_rbd_assignments (airline_id, booking_class_code, service_class, class_description, booking_priority, is_active)
SELECT 
    ac.id,
    'C',
...
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'C'
);

-- Add Business class D for discounted business fares
INSERT INTO public.airline_rbd_assignments (airline_id, booking_class_code, service_class, class_description, booking_priority, is_active)
SELECT 
    ac.id,
    'D',
...
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'D'
);

-- Add First class F for premium carriers that offer first class
INSERT INTO public.airline_rbd_assignments (airline_id, booking_class_code, service_class, class_description, booking_priority, is_active)
SELECT 
    ac.id,
    'F',
...
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'F'
);

-- Add First class A for advance purchase first class
INSERT INTO public.airline_rbd_assignments (airline_id, booking_class_code, service_class, class_description, booking_priority, is_active)
SELECT 
    ac.id,
    'A',
...
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'A'
);

-- Add Premium Economy W for airlines that offer premium economy
INSERT INTO public.airline_rbd_assignments (airline_id, booking_class_code, service_class, class_description, booking_priority, is_active)
SELECT 
    ac.id,
    'W',
...
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'W'
);

-- Add common economy fare classes for full-service carriers
-- Economy B (flexible economy)
INSERT INTO public.airline_rbd_assignments (airline_id, booking_class_code, service_class, class_description, booking_priority, is_active)
SELECT 
    ac.id,
    'B',
...
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'B'
);

-- Economy M (standard economy)
INSERT INTO public.airline_rbd_assignments (airline_id, booking_class_code, service_class, class_description, booking_priority, is_active)
SELECT 
    ac.id,
    'M',
...
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'M'
);

-- Economy H (advance purchase)
INSERT INTO public.airline_rbd_assignments (airline_id, booking_class_code, service_class, class_description, booking_priority, is_active)
SELECT 
    ac.id,
    'H',
...
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'H'
);

-- Economy K (discounted)
INSERT INTO public.airline_rbd_assignments (airline_id, booking_class_code, service_class, class_description, booking_priority, is_active)
SELECT 
    ac.id,
    'K',
...
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'K'
);

-- Economy Q (restricted)
INSERT INTO public.airline_rbd_assignments (airline_id, booking_class_code, service_class, class_description, booking_priority, is_active)
SELECT 
    ac.id,
    'Q',
...
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'Q'
);

-- Economy L (super saver)
INSERT INTO public.airline_rbd_assignments (airline_id, booking_class_code, service_class, class_description, booking_priority, is_active)
SELECT 
    ac.id,
    'L',
...
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'L'
);
...
AND NOT EXISTS (
    SELECT 1 FROM public.airline_rbd_assignments ara 
    WHERE ara.airline_id = ac.id AND ara.booking_class_code = 'N'
);
