-- Fix client access control with proper dependency management
-- Drop policies first, then function, then recreate with stricter controls
...
-- Step 6: Create audit trigger for all client access
CREATE OR REPLACE FUNCTION public.audit_all_client_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  -- Log every single access to client data
  PERFORM public.log_security_event(
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE trigger_name = 'audit_client_access_trigger'
  ) THEN
    CREATE TRIGGER audit_client_access_trigger
      AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.clients
      FOR EACH ROW EXECUTE FUNCTION public.audit_all_client_access();
  END IF;
END $$;