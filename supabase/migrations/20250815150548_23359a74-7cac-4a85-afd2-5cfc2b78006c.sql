-- Create sample data for existing users and clients
-- First, let's create some bookings with the correct schema
...
SELECT 
  ...
  jsonb_build_array(
    jsonb_build_object(
      'departure', (ARRAY['LAX', 'LHR', 'CDG', 'JFK', 'DXB', 'NRT'])[ceil(random() * 6)::int],
      'arrival', (ARRAY['LAX', 'LHR', 'CDG', 'JFK', 'DXB', 'NRT'])[ceil(random() * 6)::int],
      'airline', (ARRAY['AA', 'BA', 'AF', 'LH', 'EK', 'JL'])[ceil(random() * 6)::int],
      'flight_number', (ARRAY['AA', 'BA', 'AF', 'LH', 'EK', 'JL'])[ceil(random() * 6)::int] || (100 + random() * 900)::int,
...
-- Update client total_spent and total_bookings based on bookings
UPDATE public.clients 
SET 
  total_spent = booking_totals.total_spent,
  total_bookings = booking_totals.booking_count,
...
WHERE ur.role = 'user'
AND random() < 0.7; -- 70% chance for each day
...
WHERE ur.role = 'user' 
AND c.user_id = ur.user_id
AND random() < 0.35; -- 35% chance for each client-agent combination
...
WHERE random() < 0.5; -- 50% chance for each client
