-- COMPREHENSIVE SECURITY OVERHAUL: Fix All Critical Vulnerabilities
...
CREATE POLICY "Authenticated users: secure client access"
ON public.clients
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "Authenticated users: own profile only"
ON public.profiles
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "Authenticated users: own gmail credentials only"
ON public.gmail_credentials
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "Authenticated users: own communications only"
ON public.email_exchanges
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "Authenticated users: secure quote access"
ON public.quotes
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "Authenticated users: secure request access"
ON public.requests
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "Authenticated users: own sessions only"
ON public.user_sessions
FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "System only: session management"
ON public.user_sessions
FOR INSERT
TO authenticated
USING (false)
...
CREATE POLICY "System only: session updates"
ON public.user_sessions
FOR UPDATE
TO authenticated
USING (false)
...
CREATE POLICY "System only: session cleanup"
ON public.user_sessions
FOR DELETE
TO authenticated
USING (false);

-- Phase 5: Add comprehensive access logging triggers
...
-- Apply logging triggers to all sensitive tables
...
DROP TRIGGER IF EXISTS log_quotes_access ON public.quotes;
CREATE TRIGGER log_quotes_access
  AFTER INSERT OR UPDATE OR DELETE ON public.quotes
  FOR EACH ROW EXECUTE FUNCTION public.log_quotes_access();

-- Phase 6: Emergency access function for critical admin operations
...
-- Phase 7: Update existing access control functions
  ...
  SELECT 
    CASE 
      ...
      ELSE (
        ...
        OR EXISTS (
          -- Emergency admin access (heavily audited)
          SELECT 1 FROM public.user_roles ur
          WHERE ur.user_id = auth.uid() 
          AND ur.role = 'admin'
          AND public.emergency_admin_access('clients', 'administrative_access')
        )
      )
    END;
$$;