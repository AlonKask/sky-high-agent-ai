-- ========================================
-- CRITICAL SECURITY FIX: ENCRYPTION KEYS TABLE HARDENING (CORRECTED)
-- ========================================
...
-- Phase 3: Create private schema for system-only operations
...
-- Phase 4: Create system-only functions for encryption key management
...
-- Phase 5: Create secure encryption key rotation function (system-only)
...
-- Phase 6: Remove encryption_key_id column from clients table for security
-- Instead, use the system function to get active key
ALTER TABLE public.clients DROP COLUMN IF EXISTS encryption_key_id;

-- Phase 7: Create ultra-secure audit logging for any access attempts
...
-- Apply triggers to catch any access attempts (corrected syntax)
DROP TRIGGER IF EXISTS encryption_key_access_monitor_select ON public.encryption_keys;
DROP TRIGGER IF EXISTS encryption_key_access_monitor_modify ON public.encryption_keys;

CREATE TRIGGER encryption_key_access_monitor_select
  BEFORE SELECT ON public.encryption_keys
  FOR EACH STATEMENT EXECUTE FUNCTION public.log_encryption_key_access_attempt();

CREATE TRIGGER encryption_key_access_monitor_modify
  BEFORE INSERT OR UPDATE OR DELETE ON public.encryption_keys
  FOR EACH STATEMENT EXECUTE FUNCTION public.log_encryption_key_access_attempt();

-- Phase 8: Ensure no functions expose encryption key metadata
...
AS $$
BEGIN
  -- Only return generic status, no key metadata
