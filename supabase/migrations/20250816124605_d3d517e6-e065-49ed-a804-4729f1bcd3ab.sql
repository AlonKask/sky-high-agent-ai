...
-- This addresses the final 3 security warnings

-- Find and fix functions missing SET search_path TO 'public'
-- Based on the linter warnings, these are likely the remaining functions

-- 1. Fix get_client_data_secure function
...
-- 2. Fix emergency_client_access function if it exists
...
-- 3. Fix update_client_encrypted_field function
...
BEGIN
  ...
  IF p_field_name = 'encrypted_ssn' THEN
    UPDATE clients SET encrypted_ssn = p_value WHERE id = p_client_id;
  ELSIF p_field_name = 'encrypted_passport_number' THEN
    UPDATE clients SET encrypted_passport_number = p_value WHERE id = p_client_id;
  ELSIF p_field_name = 'encrypted_payment_info' THEN
    UPDATE clients SET encrypted_payment_info = p_value::jsonb WHERE id = p_client_id;
  END IF;
