-- First, let's check and create sample data that matches the actual schema
-- Create sample bookings using existing schema
...
-- Create agent performance metrics for last 6 months
...
WHERE ur.role = 'user'
AND random() < 0.8 -- 80% chance for each date
...
-- Create booking commissions based on confirmed bookings
...
SELECT 
  ...
  CASE 
    WHEN random() < 0.75 THEN (now() - (random() * interval '90 days'))::date
    ELSE NULL
  END as payout_date
...
-- Create some sample requests for conversion rate calculations
INSERT INTO public.requests (
  id, user_id, client_id, departure_city, arrival_city, 
...
SELECT 
  ...
  ur.user_id,
  c.id as client_id,
  (ARRAY['New York', 'London', 'Paris', 'Tokyo', 'Dubai'])[ceil(random() * 5)::int] as departure_city,
  (ARRAY['Los Angeles', 'Frankfurt', 'Hong Kong', 'Sydney', 'Singapore'])[ceil(random() * 5)::int] as arrival_city,
  (current_date + (random() * 90)::int) as departure_date,
  (current_date + (random() * 90 + 7)::int) as return_date,
  (random() * 3 + 1)::int as adults,
  (random() * 2)::int as children,
  (random() < 0.2)::int as infants,
  (ARRAY['business', 'first', 'premium_economy'])[ceil(random() * 3)::int] as preferred_class,
  CASE 
    WHEN random() < 0.3 THEN 'completed'
    WHEN random() < 0.6 THEN 'in_progress'
    WHEN random() < 0.8 THEN 'quoted'
    ELSE 'new'
  END as status,
  'Client request for premium travel options'
FROM public.user_roles ur
CROSS JOIN public.clients c
WHERE ur.role = 'user' 
AND c.user_id = ur.user_id
AND random() < 0.5 -- 50% chance for each client-agent combination
LIMIT 500
ON CONFLICT (id) DO NOTHING;