...
-- Phase 1: Zero-Trust Client Data Access & Enhanced Encryption
...
-- Enhanced clients table security trigger
    ...
    PERFORM public.log_security_monitoring(
      ...
      jsonb_build_object(
        ...
        'encrypted_fields_accessed', ARRAY[
          CASE WHEN NEW.encrypted_ssn IS NOT NULL OR OLD.encrypted_ssn IS NOT NULL THEN 'ssn' END,
          CASE WHEN NEW.encrypted_passport_number IS NOT NULL OR OLD.encrypted_passport_number IS NOT NULL THEN 'passport' END,
          CASE WHEN NEW.encrypted_payment_info IS NOT NULL OR OLD.encrypted_payment_info IS NOT NULL THEN 'payment' END
        ]
      )
    );
  END IF;
...
-- Attach enhanced security trigger
