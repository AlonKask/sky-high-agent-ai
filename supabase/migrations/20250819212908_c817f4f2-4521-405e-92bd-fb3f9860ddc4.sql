-- Fix ambiguous column reference issue in rate limiting functions
-- The issue is that window_start appears both as a function parameter and table column
...
-- Also fix the security event type constraint issue
-- Add the missing event types that are being used in the code
