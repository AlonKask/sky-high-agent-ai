-- COMPREHENSIVE SECURITY HARDENING PLAN - FIXED VERSION
...
-- Handle messages table properly
DO $$
BEGIN
...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'booking_commissions') THEN
        ...
        -- Drop existing policies if they exist
        DROP POLICY IF EXISTS "Users can view their own commission data" ON public.booking_commissions;
        DROP POLICY IF EXISTS "System can create commission data" ON public.booking_commissions;
        DROP POLICY IF EXISTS "Agents can update their own commission data" ON public.booking_commissions;
...
INSERT INTO public.security_events (
    ...
    jsonb_build_object(
        ...
        'tables_secured', ARRAY['clients', 'bookings', 'email_exchanges', 'quotes', 'gmail_credentials', 'messages'],
