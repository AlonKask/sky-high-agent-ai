...
-- First drop dependent policies, then functions, then recreate everything

-- Drop policies that depend on functions we need to recreate
...
-- Recreate satisfaction scores policy
CREATE POLICY "Enhanced satisfaction scores access" ON public.client_satisfaction_scores
FOR SELECT USING (can_access_satisfaction_scores(client_id, agent_id));
